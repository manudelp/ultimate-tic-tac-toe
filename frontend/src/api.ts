...
export const testTurnstile = async (token: string): Promise<boolean> => {
  try {
    const response = await fetch(`${API_BASE_URL}/test-turnstile`, {
      method: "POST",
...
export async function loginUser(
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/auth/login`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, password, turnstile }),
    });
    ...
    if (!response.ok) {
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        if (
          errorData.message &&
          errorData.message.toLowerCase().includes("turnstile")
        ) {
          throw new Error(
...
export const registerUser = async (
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/auth/register`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, password, username, turnstile }),
    });
