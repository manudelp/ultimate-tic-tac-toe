...
export const getBotMove = async (board: number[][][][], activeMiniBoard: number[] | null, turn: string): Promise<[number, number, number, number]> => {
    const response = await axios.post<BotMoveResponse>(`${API_URL}/get-bot-move`, {
        board,
        activeMiniBoard,
        turn,
    });
...
// User Login
export const loginUser = async (email: string, password: string): Promise<{ token: string; name: string }> => {
    try {
...
export const fetchProtectedData = async (endpoint: string): Promise<unknown> => {
