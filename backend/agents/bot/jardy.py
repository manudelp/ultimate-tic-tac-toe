    ...
    def action(self, super_board, board_to_play=None):
        ...
        if board_to_play is None:
            ...
            if minimax_move is not None:
                ...
                self.moveNumber += 1
                print(f"Jardy took {time.time() - t0:.4f} seconds to play alpha beta with depth {self.depth}, btp was None")
                print(f"Jardy Total Action time when board_to_play is None, was {time.time() - self.true_time_start:.4f} seconds")
                return r, c, r_l, c_l
            else:
        ...
        # minimax with alphabeta pruning
        print(f"Jardy is thinking with alpha beta, not iterative btp is ({a}, {b})")
        t0 = time.time()
        minimax_eval, minimax_move = self.alphaBetaModel(
            board=global_board_copy, 
            board_to_play=(a, b), 
